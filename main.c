...
int main() {
  ...
  for (indexBlock = 0; indexBlock < complete_blocks; indexBlock++) {
    ...
    // Create alpha and delta1_l with key 2
    __m128i alpha, delta1_l;
    alpha = _mm_set_epi32(complete_blocks + 1, complete_blocks + 1, complete_blocks + 1, complete_blocks + 1);
    alpha = _mm_xor_si128(alpha, L);
    // cipher alpha (2 rounds)
    alpha = _mm_xor_si128(alpha, ((__m128i *) key2.KEY)[0]);
    for (j = 1; j <= 2; j++) {
    ...
    // Calculate X_i
    X_i = _mm_xor_si128(M_i, delta1_l);
    ...
    // Calculate C_i
    C_i = _mm_xor_si128(Y_i, delta1_i);
    _mm_storeu_si128(&((__m128i*)CIPHERTEXT)[indexBlock],C_i);
  }

  // Create alpha and delta3_l with key 2
  __m128i alpha, delta3_l;
  alpha = _mm_set_epi32(complete_blocks + 3, complete_blocks + 3, complete_blocks + 3, complete_blocks + 3);
  alpha = _mm_xor_si128(alpha, L);
  // cipher alpha (2 rounds)
  alpha = _mm_xor_si128(alpha, ((__m128i *) key2.KEY)[0]);
  for (j = 1; j <= 2; j++) {
  ...
  for (indexBlock = 0; indexBlock < complete_blocks; indexBlock++) {
    ...
    // Calculate Y_i
    Y_i = _mm_xor_si128(C_i, delta1_l);
    ...
    // Calculate M_i
    M_i = _mm_xor_si128(X_i, delta1_i);
